...
if (!class_exists('srlab\classes\Ecommerce\UnitName')) :
	class UnitName
	{
		...
		public function sr_unit_field()
		{
			...
			woocommerce_wp_text_input(array('id' => 'unit_name', 'type' => 'text', 'label' => __('Unit name', 'woocommerce-max-quantity'), 'placeholder' => 'e.g. "Jaw"', 'desc_tip' => 'true', 'description' => __('Set unit name (e.g. oz)', 'moonrock'), 'custom_attributes' => [], 'value' => (isset($unit['unit_name']) && $unit['unit_name'] !== '') ? $unit['unit_name'] : '',));
		}
		...
		public function sr_append_unitname_quantity()
		{
			...
			echo "<span class='unit_name'>".strtoupper($unit)."(S)</span>";
		}
	} // End Class
endif;
